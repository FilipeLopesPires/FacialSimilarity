...
Since testing several number of centroids every time we need to generate a
codebook for an audio file is too computationally expensive, we ha to find a 
different way of testing the variation of this aspect.
...
$numCentroids = alpha x len(block)$, where {\it len(blocks)\/} is the number of
samples in a block.
...
The audio files were first preprocessed and converted to mono files with a 
quantization of 8 bits and no reduction factor.
The generated codebooks (from the initial dataset) were built with blocks of 
size 800, an overlapping factor of 0.3, an error threshold of 0.01 and with 
K-Means being run twice.
.................................
